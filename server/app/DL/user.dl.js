...
User.updateById = (id, user, result) => {
  sql.query(
    "UPDATE users SET  first_name=?, last_name=?, tz=?, email=?, address=?, phone=?, type=?, photo=?, status=? WHERE id = ?",
    [ user.first_name, user.last_name, user.tz, user.email, user.address, user.phone, user.type, user.photo, user.status, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(err, null);
