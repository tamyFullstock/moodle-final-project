...
const chechUserValidation = (req, res, next) => {
    ...
    if (!/^\d+$/.test(user.tz)) {
      res.status(400).send({message: "ID must be numeric"});
    }
    ...
    if (!user.type) {
      res.status(400).send({message:"User role is required"});
    }
  
    // Check if user.type is either "student" or "lecturer"
    const validTypes = ["student", "lecturer"];
    if (!validTypes.includes(user.type)) {
      res.status(400).send({message:"User role must be either 'student' or 'lecturer'"});
    }
    req.validUser = user;
    next();
  };
